...
def generate_color_palette(base_color: str) -> Dict[str, str]:
    ...
    palette = {
        "primary": base_color,
        "primary-light": rgb_to_hex(
            tuple(int(c * 255) for c in colorsys.hls_to_rgb(h, min(1.0, l + 0.1), s))
        ),
        "primary-dark": rgb_to_hex(
            tuple(int(c * 255) for c in colorsys.hls_to_rgb(h, max(0.0, l - 0.1), s))
        ),
        "primary-muted": rgb_to_hex(
            tuple(int(c * 255) for c in colorsys.hls_to_rgb(h, l, max(0.0, s - 0.2)))
        ),
        "primary-bright": rgb_to_hex(
            tuple(
                int(c * 255)
                for c in colorsys.hls_to_rgb(h, min(1.0, l + 0.2), min(1.0, s + 0.1))
            )
        ),
        ...
        "secondary": rgb_to_hex(
            tuple(
                int(c * 255)
                for c in colorsys.hls_to_rgb(secondary_h, secondary_l, secondary_s)
            )
        ),
        "secondary-light": rgb_to_hex(
            tuple(
                ...
                for c in colorsys.hls_to_rgb(
                    secondary_h, min(1.0, secondary_l + 0.1), secondary_s
                )
            )
        ),
        "secondary-dark": rgb_to_hex(
            tuple(
                int(c * 255)
                for c in colorsys.hls_to_rgb(
                    secondary_h, max(0.0, secondary_l - 0.1), secondary_s
                )
            )
        ),
        "secondary-muted": rgb_to_hex(
            tuple(
                int(c * 255)
                for c in colorsys.hls_to_rgb(
                    secondary_h, secondary_l, max(0.0, secondary_s - 0.2)
                )
            )
        ),
