...
def query_meus_casos(id_usuario: int):
    return (
        ...
        .filter(
            or_(
                Caso.id_usuario_responsavel == id_usuario,
                Caso.id_orientador == id_usuario,
...
def query_opcoes_filtro_meus_casos(id_usuario, opcao_filtro):
    if opcao_filtro == opcoes_filtro_meus_casos["CADASTRADO_POR_MIM"][0]:
        return query_meus_casos(id_usuario).filter(Caso.id_criado_por == id_usuario)
        ...
        return query_meus_casos(id_usuario).filter(
            Caso.situacao_deferimento == opcao_filtro
        )
    else:
        return None
...
def get_num_eventos_atual(caso_id):
    num_eventos_criados = (
        db.session.query(Evento.num_evento)
        .filter(Evento.id_caso == caso_id)
...
def get_num_lembretes_atual(caso_id):
    num_lembretes_criados = (
        db.session.query(Lembrete.num_lembrete)
        .filter(Lembrete.id_caso == caso_id)
