...
class Caso(Base):
    ...
    situacao_deferimento = db.Column(
        db.String(50, collation="latin1_general_ci"),
        nullable=False,
        default="aguardando_deferimento",
    )
    justif_indeferimento = db.Column(
        db.String(280, collation="latin1_general_ci"), nullable=True
    )
...
class Processo(Base):
    ...
    caso = db.relationship("Caso", lazy="joined")
    status = db.Column(db.Boolean, default=True, nullable=False)
    id_criado_por = db.Column(
        db.Integer, db.ForeignKey("usuarios.id"), nullable=False, default=1
    )
    criado_por = db.relationship("Usuario", foreign_keys=[id_criado_por])
