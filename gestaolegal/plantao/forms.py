...
class OrientacaoJuridicaForm(FlaskForm):
    area_direito    = SelectField('Área do Direito',
                                ...
                                validators=[
                                    ...
                                    AnyOf([area_do_direito[key][0] for key in area_do_direito], message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                    ]
                                )
...
class CadastroAtendidoForm(EnderecoForm):
    ...
    cnpj            = StringField('CNPJ',
                                validators=[
                                    RequiredIf(pj_constituida = True, message = MSG_NaoPodeEstarEmBranco.format('O cnpj')),
                                    Length(max=max_cnpj, message="Por favor, use no máximo {} caracteres para o CPF.".format(max_cnpj))
                                    ]
                                )
    ...
    como_conheceu   = SelectField('Como ficou sabendo da DAJ/Direito Vivo?',
                                ...
                                validators=[
                                    ...
                                    AnyOf([como_conheceu_daj[key][0] for key in como_conheceu_daj], message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                    ]
                                )

    indicacao_orgao = StringField('Qual foi o órgão?',
                                validators=[
                                    RequiredIf(como_conheceu = como_conheceu_daj['ORGAOSPUBLICOS'][0], message = MSG_NaoPodeEstarEmBranco.format('"Qual foi o órgão?"')),
                                    Length(max=max_indicacaoOrgao, message='Por favor, use no máximo {} caracteres para o campo "Qual foi o órgão?".'.format(max_indicacaoOrgao))
                                    ]
                                )

    procurou_outro_local = SelectField('Você procurou outro local para resolver a demanda antes de vir à DAJ?',
                                    choices=[
                                        (True,'Sim'),
                                        (False,'Não')
                                        ],
                                    coerce=lambda x: x == 'True', #https://stackoverflow.com/questions/33429510/wtforms-selectfield-not-properly-coercing-for-booleans
                                    validators=[InputRequired(MSG_SelecioneUmaOpcaoLista.format('de "Você procurou outro local para resolver a demanda antes de vir à DAJ?"'))]
                                )

    procurou_qual_local = StringField('Qual local?',
                                validators=[
                                    RequiredIf(procurou_outro_local = True, message = MSG_NaoPodeEstarEmBranco.format('"Qual local?"')),
                                    Length(max=max_procurouOutroLocal, message='Por favor, use no máximo {} caracteres para o campo "Qual local?".'.format(max_procurouOutroLocal))
                                    ]
                                )
    ...
    pj_constituida      = SelectField('Existe Pessoa Jurídica constituída?',
                        choices=[
                            (True,'Sim'),
                            (False,'Não')
                            ],
                        coerce=lambda x: x == 'True', #https://stackoverflow.com/questions/33429510/wtforms-selectfield-not-properly-coercing-for-booleans
                        validators=[InputRequired(MSG_SelecioneUmaOpcaoLista.format('de "Existe Pessoa Jurídica constituída?"'))]
                        )

    repres_legal      = SelectField('O atendido é o representante legal?',
                        choices=[
                            (True,'Sim'),
                            (False,'Não')
                            ],
                        coerce=lambda x: x == 'True', #https://stackoverflow.com/questions/33429510/wtforms-selectfield-not-properly-coercing-for-booleans
                        validators=[InputRequired(MSG_SelecioneUmaOpcaoLista.format('de "O atendido é o representante legal? "'))],
                        default = 'True'
                        )
...
###Dados Representante Legal Atendido com Pessoa Juridica Constituida
    ...
    contato_repres_legal       = StringField('Telefone de contato do representante legal:',
                                validators=[
                                    Optional(),
                                    Length(max=max_telefone, message="Por favor, use no máximo {} caracteres para o telefone fixo.".format(max_telefone))
                                    ]
    ...
    pretende_constituir_pj = SelectField('Pretende-se constituir Pessoa Jurídica?',
                                choices=[
                                    (True,'Sim'),
                                    (False,'Não')
                                    ],
...
class TornarAssistidoForm(FlaskForm):
    pj_constituida       = SelectField('Existe PJ constituída?',
                                    choices=[
                                        (True,'Sim'),
                                        (False,'Não')
                                        ],
...
############ Dados gerais de ambos tipos de assistidos ############
    ...
    raca                = SelectField('Raça/Cor (autodeclaração)',
                                    choices=[
                                    ...
                                    validators=[
                                        ...
                                        AnyOf([raca_cor[key][0] for key in raca_cor],message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                    ]
                                )
...
######################### Pessoa Física ##################
    grau_instrucao      = SelectField('Grau de instrução',
                                choices=[
                                ...
                                validators=[
                                    ...
                                    AnyOf([escolaridade[key][0] for key in escolaridade],message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                ]
                            )

    salario           = MyFloatField('Salário',
                            validators=[
                                DataRequired(MSG_NaoPodeEstarEmBranco.format('O salário')),
                                NumberRange(min = 0, max = 9999999999, message = "Valor do salário excede os limites permitidos.")
                                ]
                            )

    beneficio            = SelectField('Recebe algum benefício social?',
                            choices=[
                            ...
                            validators=[
                                ...
                                AnyOf([beneficio[key][0] for key in beneficio],message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                            ]
                        )
    ...
    contribui_inss            = SelectField('Contribui para a previdência social',
                                ...
                                validators=[
                                    ...
                                    AnyOf([contribuicao_inss[key][0] for key in contribuicao_inss],message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                ]
                            )

    qtd_pessoas_moradia         = IntegerField('Quantas pessoas moram com você?',
                                    validators=[
                                        DataRequired(MSG_NaoPodeEstarEmBranco.format('Quantas pessoas moram com você')),
    ...
    participacao_renda          = SelectField('Qual a sua posição em relação à renda familiar?',
                                    choices=[
                                ...
                                validators=[
                                    ...
                                    AnyOf([participacao_renda[key][0] for key in participacao_renda],message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                ]
                            )

    tipo_moradia                = SelectField('A família reside em:',
                                    choices=[
    ...
    possui_outros_imoveis       = SelectField('A família possui outros imóveis?',
                                        choices=[
                                            (True,'Sim'),
                                            (False,'Não')
                                        ],
                                    coerce=lambda x: x == 'True', #https://stackoverflow.com/questions/33429510/wtforms-selectfield-not-properly-coercing-for-booleans
                                    validators=[InputRequired(MSG_SelecioneUmaOpcaoLista.format('de "A família possui outros imóveis?"'))]
    ...
    possui_veiculos             = SelectField('A família possui veículos?',
                                    ...
                                    coerce=lambda x: x == 'True', #https://stackoverflow.com/questions/33429510/wtforms-selectfield-not-properly-coercing-for-booleans
                                    validators=[InputRequired(message = MSG_SelecioneUmaOpcaoLista.format('de "A família possui veículos?"'))]

    )

    possui_veiculos_obs         = StringField('Qual é o veículo?',
                                    validators=[
                                        RequiredIf(possui_veiculos = True, message = MSG_NaoPodeEstarEmBranco.format('"Qual é o veículo?"')),
                                        Length(max=max_qual_veiculo, message="Por favor, use no máximo {} caracteres para descrever a profissão.".format(max_qual_veiculo))
                                    ]
                                )

    quantos_veiculos            = IntegerField('Quantos veículos?',
                                        validators=[
                                            RequiredIf(possui_veiculos = True, message = MSG_NaoPodeEstarEmBranco.format('"Quantos veículos?"')),
                                            NumberRange(min = 0 ,max=999999999, message="Por favor, use no máximo {} numeros.".format(999999999))
                                        ]
                                    )

    ano_veiculo              = StringField('Qual o ano do veículo?',
                                    validators=[
                                        RequiredIf(possui_veiculos = True, message = MSG_NaoPodeEstarEmBranco.format('"Qual o ano do veículo?"')),
                                        Length(max=max_ano_veiculo, message="Por favor, use no máximo {} caracteres para descrever a profissão.".format(max_ano_veiculo))
                                    ]
                                )

    doenca_grave_familia    =  SelectField('Há pessoas com doença grave na família?',
                                choices=[
                                ("sim", "Sim"),
                                ("nao", "Não"),
                                ("nao_inf", "Não informou")
                                ],
                                validators=[
    ...
    pessoa_doente      = SelectField('Pessoa doente:',
                                choices=[
                                ...
                                validators=[
                                    RequiredIf(doenca_grave_familia = 'sim', message = MSG_SelecioneUmaOpcaoLista.format('"Pessoa doente:"')),
                                    AnyOf([qual_pessoa_doente[key][0] for key in qual_pessoa_doente],message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                ]
                            )

    pessoa_doente_obs   = StringField('Outros: ',
                                validators = [RequiredIf(doenca_grave_familia = 'sim', pessoa_doente = qual_pessoa_doente['OUTROS'][0], message = MSG_NaoPodeEstarEmBranco.format('"Outros"'))
                                ]
                            )
...
# # ############# Pessoa Jurídica
    ...
    socios                     = StringField('Sócios da Pessoa Jurídica',
                                validators=[
                                    Optional()
                                ]
    ...
    enquadramento      = SelectField('Enquadramento',
                                choices=[
                                ...
                                validators=[
                                    RequiredIf(pj_constituida = True, message = MSG_SelecioneUmaOpcaoLista.format(" de Enquadramento")),
                                    AnyOf([enquadramento[key][0] for key in enquadramento],message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                ]
                            )

    sede_bh             = SelectField('Sede constituída ou a constituir em Belo Horizonte?',
                                    choices=[
                                        (True,'Sim'),
                                        (False,'Não')
                                        ],
    ...
    regiao_sede_bh      = SelectField('Qual seria a região, em caso de sede em belo horizonte?',
                                choices=[
                                ...
                                validators=[
                                    RequiredIf(pj_constituida = True, sede_bh = True, message = MSG_SelecioneUmaOpcaoLista.format('de "Qual seria a região, em caso de sede em belo horizonte?"')),
                                    AnyOf([regiao_bh[key][0] for key in regiao_bh],message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                ]
                            )

    regiao_sede_outros  = StringField('Não sendo em Belo Horizonte, qual seria o local da sede constituída ou a constituir?',
                                    validators=[
                                        RequiredIf(pj_constituida = True, sede_bh = False, message = MSG_NaoPodeEstarEmBranco.format('"Não sendo em Belo Horizonte, qual seria o local da sede constituída ou a constituir?"')),
                                        Length(max=max_sede_outros, message="Por favor, use no máximo {} caracteres para descrever a profissão.".format(max_sede_outros))
                                    ]
                                )

    area_atuacao           = SelectField('Área de atuação',
                                choices=[
                                ...
                                validators=[
                                    RequiredIf(pj_constituida = True, message = MSG_SelecioneUmaOpcaoLista.format("Área de atuação")),
                                    AnyOf([area_atuacao[key][0] for key in area_atuacao],message="Desculpe, ocorreu um erro. Por favor, atualize a página.")
                                ]
                            )

    negocio_nascente            = SelectField('É negócio nascente?',
                                    choices=[
                                        (True,'Sim'),
                                        (False,'Não')
                                        ],
    ...
    orgao_registro          = SelectField('Órgão competente pelo registro do ato constitutivo',
                                    choices=[
    ...
    resultado_econ_neg      = SelectField('O resultado econômico do último ano foi negativo?',
                                    choices=[
                                        ('sim','Sim'),
                                        ('nao','Não'),
                                        ('nao_aplica','Não se aplica')
                                        ],
                                    validators=[RequiredIf(pj_constituida = True, message = MSG_SelecioneUmaOpcaoLista.format('de "O resultado econômico do último ano foi negativo?"'))]
                                )

    tem_funcionarios            = SelectField('Tem funcionários?',
                                    choices=[
                                        ('sim','Sim'),
                                        ('nao','Não'),
                                        ('nao_aplica','Não se aplica')
                                        ],
                                    validators=[RequiredIf(pj_constituida = True, message = MSG_SelecioneUmaOpcaoLista.format('de "Tem funcionários?"'))]
                                )

    qtd_funcionarios              = StringField('Em caso positivo, quantos funcionários?',
                                    validators=[
                                        RequiredIf(pj_constituida = True, tem_funcionarios = 'sim',message = MSG_NaoPodeEstarEmBranco.format('"Em caso positivo, quantos funcionários?"')),
                                        Length(max=max_qts_func, message="Por favor, use no máximo {} caracteres para descrever a profissão.".format(max_qts_func))
                                    ]
                                )
