...
class ArquivoRepository(BaseRepository[ArquivoSchema, Arquivo]):
    ...
    def create_arquivo_caso(self, data: dict) -> ArquivoCaso:
    ...
    def get_arquivos_by_caso(self, caso_id: int) -> list[ArquivoCaso]:
        result = self.arquivo_caso_repo.get(where_conditions=[("id_caso", "eq", caso_id)])
        return result.items
    ...
    def get_arquivos_by_evento(self, evento_id: int) -> list[ArquivosEvento]:
        result = self.arquivos_evento_repo.get(where_conditions=[("id_evento", "eq", evento_id)])
        return result.items
