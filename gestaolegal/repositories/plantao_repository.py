...
class PlantaoRepository(BaseRepository):
    ...
    def get_plantao(self) -> Plantao | None:
        """A tabela `plantao` guarda uma única linha, sem constraint que garanta
        isso. Lemos sempre a primeira e avisamos se houver mais de uma."""
        ...
        if len(rows) > 1:
            logger.warning(
                f"Tabela plantao tem {len(rows)} linhas; usando a de menor id"
            )
    ...
    def list_dias(self, somente_ativos: bool = True) -> list[DiaPlantao]:
    ...
    def find_dia_por_data(self, data: date) -> DiaPlantao | None:
    ...
    def create_dia(self, data: date) -> int:
        result = self.session.execute(
            insert(dias_plantao).values(data=data, status=True)
        )
        self.session.flush()
        return result.lastrowid
    ...
    def desativar_todos_dias(self) -> None:
        self.session.execute(
            sql_update(dias_plantao)
            .where(dias_plantao.c.status.is_(True))
            .values(status=False)
        )
    ...
    def list_marcacoes_ativas(self) -> list[dict[str, Any]]:
    ...
    def create_marcacao(self, data_marcada: date, id_usuario: int) -> int:
        result = self.session.execute(
    ...
    def desativar_todas_marcacoes(self) -> int:
        result = self.session.execute(
    ...
    def desativar_marcacoes_do_usuario(self, id_usuario: int) -> int:
        result = self.session.execute(
            sql_update(dias_marcados_plantao)
            .where(
                dias_marcados_plantao.c.id_usuario == id_usuario,
    ...
    def list_marcacoes_para_confirmacao(self, data: date) -> list[dict[str, Any]]:
    ...
    def find_marcacao_by_id(self, id: int) -> DiaMarcadoPlantao | None:
